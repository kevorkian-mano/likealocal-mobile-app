...
class _AdminUserManagementScreenState extends State<AdminUserManagementScreen> {
  ...
  Widget _buildActionButtons(UserModel user) {
    return PopupMenuButton<String>(
      ...
      itemBuilder: (ctx) => [
        PopupMenuItem(value: 'toggle_pro', child: Text(user.isPro ? 'Demote to Free' : 'Promote to Pro')),
        PopupMenuItem(value: 'toggle_ban', child: Text(user.isBanned ? 'Unban User' : 'Ban User'), textColor: user.isBanned ? Colors.green : Colors.red),
        const PopupMenuItem(value: 'delete', child: Text('Delete Account'), textColor: Colors.red),
      ],
    );
  }
  ...
  void _handleAction(String action, UserModel user) async {
    ...
    switch (action) {
      case 'toggle_pro':
        await FirebaseFirestore.instance.collection('users').doc(user.id).update({'isPro': !user.isPro});
        break;
      case 'toggle_ban':
        if (user.isBanned) {
